...
int main()
{
...
#if SHOW
    cv::namedWindow("stitch", cv::WINDOW_AUTOSIZE); // cv::WINDOW_NORMAL
#endif
    ...
    while (cap1.read(frame[0]) && cap2.read(frame[1]) && cap3.read(frame[2]) && cap4.read(frame[3]) && cap5.read(frame[4]) && cap6.read(frame[5]))
    {
        ...
        imagegenerate.push_back(((cv::getTickCount() - t) / cv::getTickFrequency()));
        t = cv::getTickCount();

#if YOLOv8
        RunYoloResult yolov8inf = runyolo(inf, result);
        result = yolov8inf.yoloresult;
#endif

        YOLOtime.push_back(((cv::getTickCount() - t) / cv::getTickFrequency()));
        FPS = 1 / ((cv::getTickCount() - t_f) / cv::getTickFrequency());
...
#if SAVE_RESULT
        imwrite(save_show_root + "All_image/stitch_" + str_frame_id + ".png", result);
        frame_id = frame_id + 1;
#endif
    ...
    std::cout << "Total frame number = " << Count_FPS << std::endl;
    std::cout << "Average FPS = " << Total_FPS / Count_FPS << std::endl;
    std::cout << "Finished, total time: " << ((cv::getTickCount() - app_start_time) / cv::getTickFrequency()) << " sec" << std::endl;

    float sum_imagegenerate = std::accumulate(imagegenerate.begin(), imagegenerate.end(), 0.0f);
    float average_imagegenerate = sum_imagegenerate / imagegenerate.size();

    float sum_YOLOtime = std::accumulate(YOLOtime.begin(), YOLOtime.end(), 0.0f);
    float average_YOLOtime = sum_YOLOtime / YOLOtime.size();

    std::cout << "Total frame of average_imagegenerate = " << average_imagegenerate << std::endl;
    std::cout << "Total frame of average_YOLOtime = " << average_YOLOtime << std::endl;

#if FPS_XML
    pugi::xml_document doc;
    // 創建根節點
    pugi::xml_node root = doc.append_child("FPS_data");

    // 為每個 FPS 值創建一個子節點
    for (size_t i = 0; i < fps_values.size(); ++i)
    {
        pugi::xml_node fps = root.append_child("FPS_value");
        fps.append_attribute("id") = i + 1;
        fps.text().set(fps_values[i]);
    }

    doc.save_file("FPS_data.xml");

    pugi::xml_document doc1;
    pugi::xml_node root1 = doc1.append_child("YOLOtime_data");

    // 為每個 FPS 值創建一個子節點
    for (size_t i = 0; i < YOLOtime.size(); ++i)
    {
        pugi::xml_node YOLOt = root1.append_child("YOLOtime_value");
        YOLOt.append_attribute("id") = i + 1;
        YOLOt.text().set(YOLOtime[i]);
    }

    doc1.save_file("YOLOtime_data.xml");

    pugi::xml_document doc2;
    pugi::xml_node root2 = doc2.append_child("imagegenerate_data");

    // 為每個 FPS 值創建一個子節點
    for (size_t i = 0; i < imagegenerate.size(); ++i)
    {
        pugi::xml_node gent = root2.append_child("imagegenerate_value");
        gent.append_attribute("id") = i + 1;
        gent.text().set(imagegenerate[i]);
    }

    doc2.save_file("imagegenerate_data.xml");
#endif  
    return 0;
}
